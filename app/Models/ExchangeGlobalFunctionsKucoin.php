...
class ExchangeGlobalFunctionsKucoin extends Model
{
    ...
    public function getKucoinSocketConnection()
    {
        ...
        foreach ($websocketInfo['socketUrls'] as $url)
        {
            return $url;
        }
...
//        }
    ...
    protected function initializeSocketAuthConnections(Collection $exchanges)
    {
        ...
        {
            $auth = new Auth($exchanges[$i]->api_key, $exchanges[$i]->api_secret,
                $exchanges[$i]->api_passphrase);

            $connections[$exchanges[$i]->id] = new WebSocketFeed($auth);
        }
    ...
    protected function getWebsocketInfo(array $socketConnections)
    {
        ...
        foreach($socketConnections as $key => $socketConnection)
        {
            $query = ['connectId' => uniqid('', true)];
            $res = $socketConnection->getPrivateServer($query);

            $socketUrls[$key] = $res['connectUrl'];
...
//            $blockDetector->start();
            ...
            yield $redis->set('loopIsStarted' . time(), 1, $redisOptions->withTtl(36000));
//            Log::debug('loopIsStarted', ['loopStarted' => $urls]);
            ...
            foreach ($urls as $key => $url)
            {
                ...
                Amp\Loop::repeat(200, function () use($redis, $connect, $ping)
                {
                    yield $connect->send($ping);
                });

                yield $redis->set('elderSocketIsAdded' . $key . time(), $url, $redisOptions->withTtl(36000));


                $connections[$key] = $connect;

                Amp\Loop::repeat(30000, function () use($redis, $key, $url, $ping, $connectionMessage, &$connections, $redisOptions)
                {
                    ...
                    {
                        yield $redis->set('connectionRestarted' . $key . time(), 1, $redisOptions->withTtl(36000));
...
//                    $connectionThatMustBeDeleted = yield $redis->query('hget', 'laravel_database_kucoin_disconnect_queue');
            ...
            Amp\Loop::repeat(60000, function () use ($redis, $ping, $connectionMessage, &$connections, $redisOptions)
            {
                $key = 0;
                ...
                while(true)
                {
                    ...
                    yield $redis->set('socketIsAdded', $socketUrl, $redisOptions->withTtl(36000));
                    $connections[$exchangeId] = $connect;

                    Amp\Loop::repeat(30000, function () use($redis, $exchangeId, $socketUrl, $ping, $connectionMessage, &$connections, $redisOptions)
                    {
                        $connect = $connections[$exchangeId];

                        $connectionIsExist = yield $redis->query('hexists','laravel_database_kucoin_socket_connections_elder', $exchangeId);

                        if(!$connect->isConnected() && $connectionIsExist)
                        {
                            yield $redis->set('connectionRestarted' . $exchangeId . time(), 1, $redisOptions->withTtl(36000));
                        ...
                        $flagOnDelete = yield $redis->query('hexists', 'laravel_database_kucoin_exchange_disconnect_queue', $exchangeId);
            ...
            while (true)
            {
                foreach ($connections as $key => $connection)
                {
                    ...
                    if(!empty($message))
                    {
                        ...
                        yield $redis->set('asyncCallTest'. $key . time(), $payload, $redisOptions->withTtl(36000));

//                        $this->executeKucoinOrderListenerProcessing($payload, $key);

                        Amp\asyncCall(function () use ($redis, $payload, $key, $redisOptions){

                            yield $redis->set('asyncCallPayload' . $key . time(), $payload, $redisOptions->withTtl(36000));
//
...
//                            {
                ...
                {
                    yield $redis->set('connectionIsCloseDelete'. time(), true, $redisOptions->withTtl(36000));
...
//            $blockDetector->stop();
            ...
            {
                yield $redis->set('kucoinLoopError' . time(), $e->getMessage(), $redisOptions->withTtl(36000));

                $tokenIsExpired = strpos($e->getMessage(), 'token is expired (401)');

                if($tokenIsExpired)
                {
                    $exchangeKeys = yield $redis->query('hkeys',
                        'laravel_database_kucoin_socket_connections_elder');

                    $exchanges = Exchange::whereIn('id', $exchangeKeys)->get();
                    $connections = $this->getKucoinSocketConnections($exchanges);
...
//                    }
                    ...
                    yield $redis->set('tokenIsExpired' . time(), $e->getMessage(), $redisOptions->withTtl(36000));
    ...
    protected function getKucoinSocketConnections($exchanges)
    {
        $exchanges = new Collection($exchanges);

        $socketConnections = $this->initializeSocketAuthConnections($exchanges);
        $websocketInfo = $this->getWebsocketInfo($socketConnections);

        if(!empty($socketUrls = $websocketInfo['socketUrls']))
        {
            return $socketUrls;
        }
    ...
    protected function executeKucoinOrderListenerProcessing($listenerData, $key)
    {
        $job = new OrderChangeJobKucoin($listenerData, $key);
